...
(function () {
  ...
  /* O textarea já vem com um comentário base escrito. Se a pessoa editou, o que
     ela escreveu manda; senão fica o base, que também serve de ponto de partida. */
  function setupAnswers() {
    ...
    els("textarea.answer").forEach(function (t) {
      ...
      if (typeof saved[key] === "string") t.value = saved[key];
    ...
    els("[data-restore]").forEach(function (b) {
      b.addEventListener("click", function () {
        var key = b.getAttribute("data-restore");
        var t = el('textarea.answer[data-a="' + key + '"]');
        if (!t) return;
        if (t.value.trim() !== (t.getAttribute("data-base") || "").trim() &&
            !confirm("Isso apaga o que você escreveu e volta ao comentário base. Continuar?")) return;
        t.value = t.getAttribute("data-base") || "";
        delete saved[key];
        store.set("ans", saved);
        marcarEditado(t);
        pintarStatus();
  ...
  function marcarEditado(t) {
    var base = (t.getAttribute("data-base") || "").trim();
    var editado = t.value.trim() !== base;
    t.classList.toggle("editado", editado);
    var tag = t.parentNode.querySelector(".ansflag");
    if (tag) tag.textContent = editado ? "seu comentário" : "comentário base";
  }
  ...
  function respostasAtuais() {
    var out = {};
    els("textarea.answer").forEach(function (t) {
      if (t.value.trim()) out[t.getAttribute("data-a")] = t.value;
    });
    return out;
  }

  function contarEditados() {
    return els("textarea.answer").filter(function (t) {
      return t.value.trim() !== (t.getAttribute("data-base") || "").trim();
    }).length;
  }
  ...
  function pintarStatus() {
    ...
    if (!el2) return;
    var n = contarEditados();
    var total = els("textarea.answer").length;
    var v = annIds.filter(function (id) { return done[id]; }).length;
    el2.textContent = "Você ajustou " + n + " de " + total + " comentários, e marcou " +
                      v + " de " + annIds.length + " anotações como vistas.";
  }
